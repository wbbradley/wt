...
- `F` toggles a session-only focus mode. A repository or Backburner header solos that complete scope; a branch, PR, or any detail beneath one solos its owning branch and represented descendants. The list title names the focused target, and a second `F` restores the full tree.
